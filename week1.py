...
class curve_discount_swap_points:
    ...
    def preparation_SW_data(SW_data):
        term = {"ON": pd.Timedelta("0 D").days, "W": pd.Timedelta("1 W").days, "M": pd.Timedelta("30 D").days,
                "Y": pd.Timedelta("1 Y").days}
    ...
    def sw_points_inter(self, data):
        ...
        return [[data[i], Interpolator.interpolate(self.sw_points_time, self.sw_points_value, data_new[i])] for i in
                range(len(data_new))]